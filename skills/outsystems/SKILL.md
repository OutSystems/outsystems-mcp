...
- **Use `data.category`, not message text, for error retry decisions.** Categories: `AuthError`, `ValidationError`, `UpstreamError`, `InternalError`, `CapacityError`; upstream errors also carry `data.upstream_status`. A `CapacityError` means the server is at capacity, not that the call is wrong: retry it after a short wait. Two named exceptions: a rejection naming `tenant_not_allowed`, below, and a `tenant not configured`-shaped error (routed back to setup, not retried; see First use / setup above).
- **A mentor run that fails with `code: "busy"` is transient.** Mentor saturation is reported on the run's terminal state, not as an error category, so the category rule never sees it: start the turn again in the same session after a short wait.
