...
**User-initiated.** When the user explicitly asks to report something, or when they type `/feedback`, expand to a `submit_feedback` call:
- `name`: `"user_feedback"`
...
Slash command shortcut for users (Claude Code): typing `/feedback <message>` drives `submit_feedback` per the rules above (the redaction step applies to both `value` and `rationale`; the slash command body in `commands/feedback.md` carries the same rules for the `/feedback` entry point).
