...
4. **Confirm Node.js/npx is available, then install `mcp-remote`.** This recipe requires Node.js with `npx` available on the user's machine, but what you can check yourself depends on this host's capabilities:
   ...
   - **Files, no shell:** you can't run the check above yourself. If you're on the already-configured shortcut above, there's no config entry to write — ask the user to run `npx --version` and `npm install -g mcp-remote` in a terminal, confirm Claude Desktop has been restarted since that entry was last edited (step 6), and only then walk through step 5's PATH-fallback paragraph against the existing entry if the server still won't connect. Otherwise, you can still write the config entry (step 5); ask the user to run the same two commands and report back what happened. If they report `npx` is missing, stop and tell them to install Node.js first, exactly as above; otherwise move on to step 6 (Restart).
   - **No file access at all, regardless of shell:** if you're on the already-configured shortcut above, you can still ask the user to confirm `npx --version` themselves, check whether Desktop was restarted since the entry was last edited (step 6), and hand them step 5's PATH-fallback paragraph (the `which npx` / `where npx` replacement) to apply by hand if the server still won't connect, then continue to step 7 (Authenticate) — do not send them to the from-scratch install recipe below, it does not diagnose this case. Otherwise, you cannot help set this up in this session: point the user at README.md's `Install - Claude Desktop` section (https://github.com/OutSystems/outsystems-mcp#install---claude-desktop) — but say plainly that its steps are themselves written as a prompt for an agent to run, not as instructions for a person to follow by hand, so what actually unblocks them is pasting that section into a different session or assistant that does have shell/file access. Stop here.
