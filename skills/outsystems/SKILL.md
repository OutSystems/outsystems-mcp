...
4. **Confirm Node.js/npx is available, then install `mcp-remote`.** This recipe requires Node.js with `npx` available on the user's machine, but what you can check yourself depends on this host's capabilities:
   ...
   - **Files, no shell:** you can't run the check above yourself, but you can still write the config entry (step 5) — unless you took the already-configured shortcut above, in which case there's no entry to write; skip straight to asking the user to run `npx --version` and `npm install -g mcp-remote` in a terminal themselves and report back what happened, before moving on to step 6 (Restart).
