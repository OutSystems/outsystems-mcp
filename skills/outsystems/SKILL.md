...
**Claude Desktop.** There is no tool to call and no way to sleep on this host, and no signal to act on before a call has actually been attempted — never tell the user a sign-in window has or should have opened before an OutSystems tool call has actually errored or failed to return. If a tool call errors (e.g. an `AuthError`-shaped rejection, or a `tenant not configured`-shaped error) or doesn't return, retry it once: `mcp-remote` may need a moment to open a browser sign-in window, whether this is the session's first tool call or a token just expired. If it's still failing after the retry, ask the user whether a sign-in window opened and whether they completed it:

- They say they completed it: retry once more. If it still errors, stop and surface it per https://github.com/OutSystems/outsystems-mcp#troubleshooting.
- They say a window opened but they haven't finished yet: ask them to complete it, then retry.
- They say no window ever appeared, that it kept reopening without ever completing, or they can't tell: treat it as the `mcp-remote` proxy's stale-registration wedge. Stop, point them at the saved-proxy-registration clear under https://github.com/OutSystems/outsystems-mcp#reset, and at https://github.com/OutSystems/outsystems-mcp#troubleshooting for the matching symptom rows. Do not re-trigger sign-in again this session.
...
- **Use `data.category`, not message text, for error retry decisions.** Categories: `AuthError`, `ValidationError`, `UpstreamError`, `InternalError`; upstream errors also carry `data.upstream_status`. Two named exceptions: the external-library `Server is busy, retry shortly` case under Caveats, and a rejection naming `tenant_not_allowed`, below.
