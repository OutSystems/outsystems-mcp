...
5. **Add the server entry.** Locate the config file: macOS `~/Library/Application Support/Claude/claude_desktop_config.json`, Windows `%APPDATA%\Claude\claude_desktop_config.json`, Linux `~/.config/Claude/claude_desktop_config.json`. Read it first (start from `{}` if it doesn't exist) and preserve every existing key — other entries in this file may carry other MCP servers' credentials in their `args`/`env`, so don't paste the full file contents back to the user or into a message when confirming the edit; quote only the `outsystems` entry you're adding or changing. Then add or replace the `mcpServers.outsystems` entry:
   ...
   Claude Desktop launches processes with a minimal PATH, so `npx` may fail to resolve even though it works in a terminal; if the server doesn't connect after restart, replace `"npx"` with the absolute path from `which npx` / `where npx`. If it still doesn't connect after that, stop and point the user at `README.md`'s Troubleshooting section rather than making further blind changes.
...
**Claude Desktop:** there is no tool to call — the user's own request already triggered `mcp-remote`'s browser sign-in window. Wait briefly, then proceed once the real tools appear. You can't observe whether that window opened or whether the user finished in it, so don't gate silently on either: if the tools still aren't there after a short wait, tell the user what you're waiting for and ask whether they completed sign-in in the browser. Retry the user's original request once they say yes; if they say no, or the window never appeared, treat it as a possible stale-proxy wedge per Reactive below rather than retrying blind.

**Reactive.** On `data.category: "AuthError"` mid-session (token expired, refresh denied, etc.): **Claude Code**, call `mcp__outsystems__authenticate` again, then retry the original call ONCE. **Claude Desktop**, there is no tool to call: retry the failed call once, give `mcp-remote` a few seconds to reopen its browser sign-in window, and only treat the retry as failed if the window never reopens or the tools still error afterward. If the window keeps reopening without ever completing sign-in, that is the `mcp-remote` proxy's `EADDRINUSE` wedge, not a normal retry: stop and point the user at the saved-proxy-registration clear under https://github.com/OutSystems/outsystems-mcp#reset rather than retrying again. If the rejection names `tenant_not_allowed`, do not re-trigger sign-in, see Rules.
