...
**Claude Desktop:** there is no tool to call. Make the OutSystems tool call that triggered this setup and wait for the user to complete sign-in in the browser `mcp-remote` opens, then proceed once the real tools appear. If no browser window opens within a few seconds, or the user closes it before finishing sign-in, tell the user immediately rather than waiting silently, and retry the same tool call once they confirm sign-in is done.

**Reactive.** On `data.category: "AuthError"` mid-session (token expired, refresh denied, etc.): **Claude Code**, call `mcp__outsystems__authenticate` again, then retry the original call ONCE. **Claude Desktop**, there is no tool to call. Retry the failed tool call once and wait for `mcp-remote` to reopen its browser sign-in window before treating the retry as failed, the same way the Lazy clause above waits on first use. If the window keeps reopening without ever completing sign-in, that is the `mcp-remote` proxy's `EADDRINUSE` wedge, not a normal retry: stop and point the user at the saved-proxy-registration clear under https://github.com/OutSystems/outsystems-mcp#reset rather than retrying again. If the rejection names `tenant_not_allowed`, do not re-trigger sign-in, see Rules.
...
**User asking how to give feedback.** When the user asks "how do I file a bug?" / "how do I give feedback?" / "how do I report a problem?", explain that `submit_feedback` is the surface AND offer to invoke it directly. On Claude Code you can also tell the user to type `/outsystems-feedback` for a guided form, or `/outsystems-feedback <message>` to submit directly: "You can type `/outsystems-feedback` for a guided form, or `/outsystems-feedback <message>` to submit directly. Want me to open the guided form now?" This is an exception to "don't volunteer" — the user asked; walking them through it is helpful, not manipulative.
