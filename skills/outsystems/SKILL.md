...
If the `outsystems` MCP tools aren't visible in your toolset, or a call returns `tenant not configured` / connection errors, the MCP server has no tenant to talk to yet. On Claude Code the plugin declares the server itself and builds its URL from the `tenant_hostname` plugin option; on Claude Desktop's Chat tab the server comes from `claude_desktop_config.json`. A rejection naming `tenant_not_allowed` is a different condition and usually not a setup fault, so check the configured host first: `claude mcp list` on Claude Code (the plugin's server is listed as `plugin:outsystems:outsystems`, a registration from an older recipe as `outsystems`; `claude mcp get outsystems` reads only the latter), or the `mcpServers.outsystems` entry in `claude_desktop_config.json` on Claude Desktop (same file the Desktop setup steps below write to; if you cannot read files in this host, ask the user to open that file and paste only the `outsystems` entry, telling them the file's other entries may hold other MCP servers' credentials in their `args`/`env` and should not be pasted), see Rules. Otherwise, do this once per machine. Pick the branch by what this host gives you: if you can run shell commands and the `claude` CLI exists, use the Claude Code recipe below; otherwise use the Claude Desktop recipe.
...
4. **Set the tenant on the plugin.** If you cannot run shell commands here, you are not in Claude Code — use the Claude Desktop recipe below instead. If `claude plugin list` shows no `outsystems`, the plugin is absent and you are reading this skill some other way: register the server directly with `claude mcp add -s user --transport http outsystems <URL>` instead of the command below. Otherwise, first check whether a tenant is already stored: `grep -n tenant_hostname ~/.claude/settings.json`. If one is, and it differs from `<TENANT>`, restate "change the plugin's tenant from X to Y" and wait for explicit confirmation, because the command below overwrites the stored value and keeps no record of the old one. Then run:
   ```bash
   claude plugin install outsystems@outsystems --config tenant_hostname=<TENANT>
   ```
   Pass the bare hostname from step 2, not `<URL>`: `--config` does not validate the value, and a scheme or path in it yields a server URL that cannot resolve. On an already-installed plugin the command answers `already installed` and nothing else; the option is written all the same. If `claude` rejects `--config` (older Claude Code), tell the user to update Claude Code, or, failing that, to run `/plugin configure outsystems@outsystems` and type `<TENANT>` there. Whichever command you ran, do NOT pass `--client-id` or `--callback-port`. The server supports OAuth Dynamic Client Registration, so the host registers its own client on an ephemeral loopback port. Then run `claude mcp list`: a lone `outsystems` line with no `plugin:outsystems:outsystems` means an entry from an older recipe is hiding the plugin's server at the same URL, and two lines with different URLs mean both are live. In either case run `claude mcp get outsystems` to read that entry's `Scope` and `URL`, tell the user both, and, with their confirmation, remove it with `claude mcp remove outsystems -s <that scope>` when the scope is `user` or `local`; never touch a `project` scope yourself, report it instead. Then tell them to restart Claude Code: the running session keeps the registration it started with, and the removed entry's saved OAuth tokens go with it, so one sign-in follows.
...
This recipe stays the Desktop path. The plugin's own server declaration depends on the `tenant_hostname` plugin option, which Desktop may not prompt for; if the Code tab reports that option unset, the remedy is the terminal `--config` command from the Claude Code recipe above, not this file.
...
If the user already has an `outsystems` MCP server configured but pointing at the wrong tenant, follow the same flow. The patches are idempotent for the same tenant and update the URL for a new one; on Claude Code the plugin's tenant changes by re-running the `--config` command in step 4, after the confirmation that step requires.
...
- **Claude Code** exposes two deferred tools the agent drives:
  ...
  When the server is the one the plugin declares (the default since 0.20.0), Claude Code namespaces the server's tools under the plugin (`mcp__plugin_outsystems_outsystems__<tool>`), and the two auth tools may carry that prefix too. The rest of this section uses the short names; match whichever prefix your toolset shows, and never call a name that isn't listed.
...
**One exception, at most once per session, on Claude Code:** an error saying the OAuth callback port is already in use is a local port conflict, not a server fault, so don't file it. It is often a fixed port pinned by an older setup. The plugin's own server never pins a port, so the pin can only live in a user-scope `outsystems` entry from an older recipe. Diagnose first with `claude mcp get outsystems`, reading the reported `URL`, `Scope` and `OAuth` line. If it answers that no server named `outsystems` exists, only the plugin's server is configured and the pin is not in the MCP config: take the branch below for the case where no `callback_port` is reported.
...
- **Uploading an external library has a 50 MB decoded cap (~67 MB encoded `zip_b64`) and per-replica concurrency gating.** Pre-flight rejects oversize payloads. The concurrency gate is not a queue: an upload arriving while all per-replica permits are held waits 5 seconds and is then rejected with `Server is busy, retry shortly`. Current servers return it as a `CapacityError` with `data.code = "server_busy"`; older servers may still return it as a `ValidationError` with the same message, where the category alone reads as fatal. Both forms are transient: retry with backoff instead of reporting a permanent failure.
...
- **Use `data.category`, not message text, for error retry decisions.** Categories: `AuthError`, `ValidationError`, `UpstreamError`, `InternalError`, `CapacityError`; upstream errors also carry `data.upstream_status`. `CapacityError` (such as `data.code = "server_busy"`) is transient: the server is at capacity and nothing is wrong with the call, so retry with backoff. Three named exceptions: the external-library `Server is busy, retry shortly` upload rejection returned as a `ValidationError` by older servers (transient like its current `CapacityError` form, so retry with backoff; see Caveats), a rejection naming `tenant_not_allowed`, below, and a `tenant not configured`-shaped error (routed back to setup, not retried; see First use / setup above).
...
**User-initiated.** When the user explicitly asks to report something, or, on Claude Code, when they type `/outsystems-feedback`, expand to a feedback call:
- `name`: `"user_feedback"`
...
Slash command shortcut for users (Claude Code): typing `/outsystems-feedback <message>` drives the feedback tool per the rules above (the redaction step applies to both `value` and `rationale`; the slash command body in `commands/outsystems-feedback.md` carries the same rules for the `/outsystems-feedback` entry point). The `outsystems-` prefix avoids the collision with Claude Code's built-in `/feedback`, which routes to Anthropic's issue tracker and would shadow an unprefixed plugin command.
