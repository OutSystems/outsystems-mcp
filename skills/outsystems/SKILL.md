...
If `mcpServers.outsystems` already exists in the config file (step 5 below) and its URL matches the tenant you mean, skip straight to step 7 (Authenticate) — the setup steps below have nothing to add.
...
4. **Confirm Node.js/npx is available, then install `mcp-remote`.** If you cannot run commands or read/write files in this host at all, hand the user the Claude Desktop install steps at https://github.com/OutSystems/outsystems-mcp#install---claude-desktop to run themselves, and stop here — don't attempt the checks below. Otherwise, this recipe requires Node.js with `npx` available on the user's machine: confirm it (e.g. `npx --version`), then run `npm install -g mcp-remote` — idempotent and safe even if it's already installed. If the command runs but reports npx missing, stop here and tell the user to install Node.js first; writing the config entry below without it produces a server that silently fails to connect.
...
- **Claude Desktop** exposes no agent-callable auth tool. The `mcp-remote` local proxy runs the OAuth flow in its own browser window on the first tool call. **Don't look for an `authenticate` tool** on Desktop, it isn't there; see Lazy below for what to do instead.
...
**Claude Desktop:** there is no tool to call and no way to sleep on this host — the user's own request already triggered `mcp-remote`'s browser sign-in window. End your turn, tell the user a sign-in window should have opened, and ask them to say when they've completed it (or if none ever appeared). When they reply: if the real tools are now present, retry their original request. If they say they haven't finished yet, ask them to complete it and retry once they confirm. If they say no window ever appeared, stop: that is the `mcp-remote` proxy's stale-registration wedge, not something a retry fixes. Point them at the saved-proxy-registration clear under https://github.com/OutSystems/outsystems-mcp#reset, and at https://github.com/OutSystems/outsystems-mcp#troubleshooting for the matching symptom rows. Do not re-trigger sign-in again this session.

**Reactive.** On `data.category: "AuthError"` mid-session (token expired, refresh denied, etc.): **Claude Code**, call `mcp__outsystems__authenticate` again, then retry the original call ONCE. **Claude Desktop**, there is no tool to call: retry the failed call once, giving `mcp-remote` a few seconds to reopen its browser sign-in window. If the retry still errors, ask the user whether a sign-in window opened and whether they completed it. If they say it kept reopening without ever completing, or they can't tell, treat it as the `mcp-remote` proxy's `EADDRINUSE` wedge: stop and point them at the saved-proxy-registration clear under https://github.com/OutSystems/outsystems-mcp#reset rather than retrying again. If they say they completed it, retry once more; if it still errors, stop and surface it per https://github.com/OutSystems/outsystems-mcp#troubleshooting. If the rejection names `tenant_not_allowed`, do not re-trigger sign-in, see Rules.
