...
If `mcpServers.outsystems` already exists in the config file (step 5 below) and its URL matches the tenant you mean, skip steps 1-3 and step 5's config write — the tenant-collection and config-write steps have nothing to add. Still work through step 4 (confirm Node.js/npx is available), step 5's PATH-fallback paragraph (Desktop launches `npx` with a minimal PATH of its own, so it can fail to resolve there even though step 4's check, run in your own shell, passed), and confirm with the user that Claude Desktop has been restarted since that entry was last edited (step 6) before concluding the missing tools are an auth problem: an unresolvable `npx` inside Desktop's launch environment, and a Desktop process that predates the config edit, are the dominant real causes of "the entry exists but the tools are still missing," and both go unnoticed if you jump straight to step 7.
...
4. **Confirm Node.js/npx is available, then install `mcp-remote`.** This recipe requires Node.js with `npx` available on the user's machine, but what you can check yourself depends on this host's capabilities:
   ...
   - **Files, no shell:** you can't run the check above yourself. If you're on the already-configured shortcut above, there's no config entry to write — ask the user to run `npx --version` and `npm install -g mcp-remote` in a terminal, then walk through step 5's PATH-fallback paragraph against the existing entry before step 6 (Restart). Otherwise, you can still write the config entry (step 5); ask the user to run the same two commands and report back what happened, then move on to step 6 (Restart).
   - **No file access at all, regardless of shell:** if you're on the already-configured shortcut above, you can still ask the user to confirm `npx --version` themselves, check whether Desktop was restarted since the entry was last edited (step 6), and hand them step 5's PATH-fallback paragraph (the `which npx` / `where npx` replacement) to apply by hand if the server still won't connect — do not send them to the from-scratch install recipe below, it does not diagnose this case. Otherwise, you cannot help set this up in this session: point the user at README.md's `Install - Claude Desktop` section (https://github.com/OutSystems/outsystems-mcp#install---claude-desktop) — but say plainly that its steps are themselves written as a prompt for an agent to run, not as instructions for a person to follow by hand, so what actually unblocks them is pasting that section into a different session or assistant that does have shell/file access. Stop here.
