...
- **A session runs one turn at a time.** Don't start another prompt on a session until the current run is terminal. A start that appears to do nothing while a run is live means poll the run you already have, not start again.
