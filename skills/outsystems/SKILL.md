...
2. **Normalize, then validate.** Accept whatever the user gives you (URL, hostname, hostname-with-path). Strip the scheme (`https://`, `http://`), any leading `www.`, trailing slash, and any path or query — keep only the host. The result must match `^[A-Za-z0-9]([A-Za-z0-9.-]*[A-Za-z0-9])?$` — if it doesn't, ask again; do not proceed with a value that fails the check, since the Windows variant below depends on it.
...
If `mcpServers.outsystems` already exists in the config file (step 5 below) and its URL matches the tenant you mean, skip steps 1-3 and 5 — the tenant-collection and config-write steps have nothing to add. Still work through step 4 (confirm Node.js/npx is available) and confirm with the user that Claude Desktop has been restarted since that entry was last edited (step 6) before concluding the missing tools are an auth problem: a missing or unresolvable `npx`, and a Desktop process that predates the config edit, are the dominant real causes of "the entry exists but the tools are still missing," and both go unnoticed if you jump straight to step 7.
...
4. **Confirm Node.js/npx is available, then install `mcp-remote`.** This recipe requires Node.js with `npx` available on the user's machine, but what you can check yourself depends on this host's capabilities:
   ...
   - **Files, no shell:** you can't run the check above yourself, but you can still write the config entry (step 5). Do that, then ask the user to run `npx --version` and `npm install -g mcp-remote` in a terminal themselves and report back what happened, before moving on to step 6 (Restart).
   - **No file access at all, regardless of shell:** you cannot help set this up in this session. Point the user at README.md's `Install - Claude Desktop` section (https://github.com/OutSystems/outsystems-mcp#install---claude-desktop) — but say plainly that its steps are themselves written as a prompt for an agent to run, not as instructions for a person to follow by hand, so what actually unblocks them is pasting that section into a different session or assistant that does have shell/file access. Stop here.
5. **Add the server entry.** Locate the config file: macOS `~/Library/Application Support/Claude/claude_desktop_config.json`, Windows `%APPDATA%\Claude\claude_desktop_config.json`, Linux `~/.config/Claude/claude_desktop_config.json`. Read it first (start from `{}` if it doesn't exist) and preserve every existing key — other entries in this file may carry other MCP servers' credentials in their `args`/`env`, so don't paste the full file contents back to the user or into a message when confirming the edit; quote only the `outsystems` entry you're adding or changing. Then add or replace the `mcpServers.outsystems` entry, picking the shape below by the user's OS — determine it yourself if you can, otherwise ask:
   ...
   Claude Desktop launches processes with a minimal PATH, so `npx` may fail to resolve even though it works in a terminal; if the server doesn't connect after restart: macOS/Linux, replace the `"command"` value `"npx"` with the absolute path from `which npx`. Windows, replace the bare `npx` token inside the `/c` argument string with the `.cmd` path from `where npx` (take the line ending in `.cmd`), quoting it if it contains a space — it commonly does (e.g. `C:\Program Files\nodejs\npx.cmd`). If it still doesn't connect after that, stop and point the user at https://github.com/OutSystems/outsystems-mcp#troubleshooting rather than making further blind changes.
...
- **`tenant_not_allowed`** (a bare 403, no `data.category`): not an auth fault. Do not treat it as a sign-in problem. Do not retry, except once if the user tells you the tenant has since been enabled, per Rules. See Rules and https://github.com/OutSystems/outsystems-mcp#troubleshooting.
- **A `tenant not configured`-shaped error, or the `outsystems` tools still aren't visible at all:** a setup fault, not an auth fault. Go back to the Claude Desktop setup steps above (step 4: npx/`mcp-remote` availability; step 6: whether Desktop was restarted since the config edit) instead of retrying the call.
- **Anything else that errors, or doesn't return** (an `AuthError`-shaped rejection, a transport/connect failure, or a call that just hangs): retry it once — `mcp-remote` may need a moment to open a browser sign-in window, whether this is the session's first tool call or a token just expired. If it's still failing after the retry, ask the user whether a sign-in window opened and whether they completed it:
  ...
  - They say no window ever appeared, or they can't tell: stop and point them at https://github.com/OutSystems/outsystems-mcp#troubleshooting (the "Server Disconnected" / "failed authorization" row covers this).
...
- **Go straight to the task.** No setup checks, no auth pre-flight beyond the lazy authentication steps described above (including Claude Desktop's read-only first call, which exists only to trigger sign-in); identity comes from the harness-negotiated bearer.
...
- **Use `data.category`, not message text, for error retry decisions.** Categories: `AuthError`, `ValidationError`, `UpstreamError`, `InternalError`; upstream errors also carry `data.upstream_status`. Three named exceptions: the external-library `Server is busy, retry shortly` case under Caveats above, a rejection naming `tenant_not_allowed` (next rule, below), and a `tenant not configured`-shaped error (routed back to setup, not retried; see First use / setup above).
...
- If the user says yes, ask for a one-line description, and submit via `submit_feedback` with `name: "user_feedback"`, `value: "bug-report"` (already known, don't ask), and `rationale` set to their description, per the user-initiated field rules above. On Claude Code, this can also be done by invoking the guided form in `commands/outsystems-feedback.md`, skipping its category-picker step since the category is already known.
