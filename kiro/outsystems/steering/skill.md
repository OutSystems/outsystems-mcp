...
**Scope — HARD BLOCK.** You may fire `agent_observation` ONLY when a specific OutSystems MCP tool call (a `mcp__outsystems*__*` tool: `mentor_*`, `deploy_*`, `publish_*`, `env_*`, `app_*`, `extlib_*`, `db_query`, `context_*`) is the subject of the observation. The following are NEVER fireable, regardless of how well they seem to match a categorical — treat them as absolute blockers:
- Invoking a Claude Code Skill (the `Skill` tool) or getting a surprising result from one.
- Reading, grepping, or otherwise navigating SKILL.md, `commands/*.md`, `.claude/**`, or any plugin/skill/rule guidance file.
- Any Bash / Read / Glob / Grep / ToolSearch / Task tool call in your session.
- Your own reasoning, planning, decision-making, or the process of figuring out what to do.
- Ambient conversational messages that don't ask you to touch any OutSystems tool ("thanks", "OK", "that's all", "sounds good", "understood").

**Enforcement self-test** — mandatory before every fire. Read your draft rationale sentence and check whether the primary subject is:
- an OutSystems MCP tool call by name (`env_info`, `publish_start`, `mentor_publish`, `app_list`, etc.) → OK to continue.
- a Skill, SKILL.md, plugin, guidance file, Claude Code internal tool, your own workflow, or a conversational close → **STOP. DO NOT FIRE.** No exceptions, no "but the categorical still fits". The blockers above override every categorical.
