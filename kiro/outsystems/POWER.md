...
There is no CLI to install. There is no OML on disk. OML stays server-side; you edit through the mentor flow (start or resume a turn, then poll it) and ship via the publish tool. The exact tool names vary by tenant — see Workflow 2.
...
Some tenants expose a different Mentor tool set: a dedicated tool to open a session (`mentor_start_session`) and a separate one to send follow-up prompts (`mentor_prompt`), a dedicated cancel tool (`mentor_cancel_prompt`), a publish tool named `mentor_publish`, and extra tools for creating or loading an app asset through Mentor (`mentor_create_asset`, `mentor_load_asset`) or uploading a file / closing the session (`mentor_request_upload`, `mentor_close_session`). `mentor_get_run` for polling is shared across both tool sets. Check `tools/list` for what your tenant has; the steps below assume the combined tool set.
