...
- **`servers` vs `mcpServers`.** VS Code and Visual Studio read `servers`; Copilot CLI, Kiro, Claude Code's root `.mcp.json`, Claude Desktop, and Cursor CLI read `mcpServers`. `copilot/mcp.json` carries both keys so each surface copies the one it needs. Writing the wrong key fails silently: the file still parses and no server appears. (Cursor also has a `.vscode/mcp.json` (IDE-only) that uses `servers`, but the CLI ignores it.)
...
Setup steps legitimately diverge between the harnesses (Claude Code sets the `tenant_hostname` plugin option, with `claude mcp add` as the fallback, Kiro Power patches `~/.kiro/settings/mcp.json`, GitHub Copilot uses VS Code/Visual Studio settings or Copilot CLI config, the root SKILL.md describes the wire-level tool names without prescribing a host). The lockstep rule applies to `## Rules` and to behavioral guidance outside the setup recipe itself, not to host-specific install recipes.
...
Since 0.20.0 the root `.mcp.json` declares the `outsystems` HTTP server with `"url": "https://${user_config.tenant_hostname}/mcp"`, and `.claude-plugin/plugin.json` declares the `tenant_hostname` option under `userConfig` (`required: true`). Facts that constrain edits here, all checked on Claude Code 2.1.236:

- The option is set at install time with `claude plugin install outsystems@outsystems --config tenant_hostname=<host>`, or in a session with `/plugin configure outsystems@outsystems` (the command Claude Code itself prints after an install that lacks the option). Re-running the install command on an installed plugin answers `already installed` and updates the option. Claude Code stores it in `~/.claude/settings.json` under `pluginConfigs["outsystems@outsystems"].options.tenant_hostname`; per the Claude Code plugin reference, project-scope settings are ignored for plugin options, so a cloned repo cannot supply the value.
- Nothing validates the value. `tenant_hostname=https://x/mcp` is stored as-is and yields `https://https://x/mcp/mcp`, so every recipe must normalize to a bare hostname before passing it, and every place that re-runs the command on an installed plugin must confirm a tenant change with the user first, because the old value is overwritten without a trace.
- With the option unset, Claude Code skips the server silently: `claude mcp list` prints `No MCP servers configured` (or simply lacks the `plugin:outsystems:outsystems` line) and the only visible message is `Plugin MCP server error - generic-error: Plugin option "tenant_hostname" isn't set. Open /plugin manage to configure it` in `claude --debug` output. The README troubleshooting table describes both surfaces; keep the quoted string verbatim.
- `claude mcp list` names the server `plugin:outsystems:outsystems`. Its tools are namespaced `mcp__plugin_outsystems_outsystems__<tool>`, so any hook matcher or permission rule written against `mcp__outsystems__*` misses the plugin-declared server. Whether Claude Code's synthesized `authenticate` / `complete_authentication` tools carry that prefix is unverified; the skill doc says so. A user-scope `outsystems` entry from the older `claude mcp add` recipe takes precedence over the plugin's server when both point at the same URL (the plugin line then disappears from `claude mcp list`); with different URLs both load.
- The Desktop app and the VS Code extension may not prompt for `userConfig` values when installing a plugin ([anthropics/claude-code#89749](https://github.com/anthropics/claude-code/issues/89749), open at the time of writing), which is why the README keeps the terminal `--config` command as the remedy and why the Claude Desktop Chat tab still uses the `mcp-remote` local proxy.
- For the three testing facts (validate does not read `.mcp.json`; the checkout's `.mcp.json` doubles as project MCP config and is neutralised by `.claude/settings.json`; isolate tests with `CLAUDE_CONFIG_DIR`), see CONTRIBUTING.md's Claude Code testing section rather than duplicating them here.
