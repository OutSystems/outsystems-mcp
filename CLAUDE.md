...
**Codex's own sandbox blocks the setup commands, and one of them fails silently.** Anything Codex runs is sandboxed, and `~/.codex/` is outside the writable root, so `codex mcp add` fails loudly (`failed to persist config … Operation not permitted`, non-zero exit) while `codex mcp login` reports `Successfully logged in` and stores no token — the OAuth flow is network plus loopback and genuinely succeeds; only the token write is denied. The user discovers it a session later as `requires OAuth reauthentication` / `MCP startup incomplete`. Every Codex recipe here therefore hands the sign-in to the user to run in their own terminal, and verifies via the `Auth` column of `codex mcp list` rather than `Status`, which reads `enabled` for a merely-registered server. Don't "simplify" a recipe by letting the agent run the login.
