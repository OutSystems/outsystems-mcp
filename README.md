...
Step 1: ask me for my OutSystems tenant hostname (something like `mycompany.outsystems.dev`).
Step 2: run `codex mcp add outsystems --url https://<my-tenant>/mcp` (substitute my actual tenant for `<my-tenant>`). Pass no `--oauth-client-id`: the server supports OAuth Dynamic Client Registration, so Codex registers its own client on an ephemeral loopback port. If that command fails with `Operation not permitted`, it was blocked by your sandbox — give me the exact command and let me run it in my own terminal instead.
Step 3: do NOT run the sign-in yourself. Tell me to run `codex mcp login outsystems` in my own terminal, outside this session, and to complete the browser sign-in there. Run from inside a session it reports success and silently stores no token, because it cannot write to `~/.codex/`. Wait for me to confirm I'm done.
Step 4: run `codex plugin list` and `codex mcp list`. On the MCP table read the `Auth` column, not just `Status`: `Status: enabled` only means the server is registered, and if `Auth` says `Not logged in` the sign-in did not stick, so send me back to Step 3. Once `Auth` looks right, tell me to start a new session and ask you anything OutSystems-related.
```
...
| `codex mcp login` says "Successfully logged in", but the next session says `requires OAuth reauthentication` / `MCP startup incomplete` (Codex) | The sign-in was run from inside a Codex session. The OAuth flow itself is network plus a loopback callback, so it completes, but storing the token needs a write to `~/.codex/` and the sandbox denies that with `Operation not permitted` — the command reports success anyway. Run `codex mcp login outsystems` yourself in a plain terminal, outside any Codex session. Confirm it stuck with `codex mcp list`: the `Auth` column must not say `Not logged in`. `Status: enabled` alone only means the server is registered. |
