...
| Nothing connects on Windows, or `npx` is "not found" | Applies to the Claude Desktop local-proxy install only. Claude Desktop launches processes with a minimal PATH. macOS/Linux: replace the `"command"` value `"npx"` with the full path from `which npx`. Windows: replace the bare `npx` token inside the `/c` argument string with the `.cmd` path from `where npx` (the line ending in `.cmd`), quoting it if it contains a space (e.g. `"C:\Program Files\nodejs\npx.cmd"`). |
