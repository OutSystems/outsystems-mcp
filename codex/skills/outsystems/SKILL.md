...
4. **Register the server.** Prefer the CLI, which writes `~/.codex/config.toml` for you:
   ...
   ```
   Pass no `--oauth-client-id` and no fixed callback port: the server advertises OAuth discovery and Dynamic Client Registration, so Codex registers its own client on an ephemeral loopback port. If you edit `~/.codex/config.toml` by hand instead, read first and preserve every other entry — never clobber siblings. The canonical shape, also kept in the repo at `codex/mcp.toml`:
   ```toml
...
5. **Sign in**: run `codex mcp login outsystems` and have the user complete the browser sign-in. See Authenticating for the recovery paths when it fails.
6. **Verify** with `codex mcp list` that `outsystems` is registered, then **retry the user's original request**.
...
**Reactive.** On `data.category: "AuthError"` mid-session (token expired, refresh denied, etc.): the stored grant lapsed — re-run `codex mcp login outsystems` (or have the user hit **Authenticate** in the desktop app), then retry the original call ONCE. If the rejection names `tenant_not_allowed`, do not re-trigger sign-in, see Rules.
