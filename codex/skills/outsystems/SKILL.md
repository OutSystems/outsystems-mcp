...
This plugin ships the skill, not the tenant URL — the tenant is per-user, so the `outsystems` MCP server has to be registered once on this machine before any tool exists to call. Codex keeps MCP configuration in `~/.codex/config.toml`, shared by Codex CLI, the ChatGPT desktop app, and the IDE extension, so registering once covers all three. The edit is idempotent, so re-run it to point at a different tenant.

Trigger conditions (any of):
- The user asks for anything OutSystems-related AND no `outsystems` MCP server is registered.
- A call returns `tenant not configured` / connection errors. A rejection naming `tenant_not_allowed` is a different condition and usually not a setup fault, so check the configured host in `~/.codex/config.toml` first, see Rules.
...
4. **Register the server.** Prefer the CLI, which writes `~/.codex/config.toml` for you:
   ```bash
   codex mcp add outsystems --url https://<TENANT>/mcp
   ```
   Pass no `--oauth-client-id` and no fixed callback port: the server advertises OAuth discovery and Dynamic Client Registration, so Codex registers its own client on an ephemeral loopback port. **This writes to `~/.codex/`, which is outside your sandbox**, so it fails with `failed to persist config … Operation not permitted` unless the host escalates it. That failure is loud and safe: if you see it, hand the exact command to the user to run in their own terminal rather than retrying or hand-editing. If you do edit `~/.codex/config.toml`, read first and preserve every other entry — never clobber siblings. The canonical shape, also kept in the repo at `codex/mcp.toml`:
   ```toml
   [mcp_servers.outsystems]
   enabled = true
   url = "https://<TENANT>/mcp"
   ```
   The key is `mcp_servers` in **TOML**, not the `mcpServers` JSON spelling every other harness uses. The wrong spelling still parses and registers nothing.
5. **Hand the sign-in to the user. Do not run `codex mcp login` yourself.** Tell them to run it in their own terminal, outside this session:
   ```bash
   codex mcp login outsystems
   ```
   This one fails *silently* in your sandbox: the OAuth flow is network and loopback, so the browser sign-in succeeds and the command prints `Successfully logged in to MCP server 'outsystems'` — but persisting the token needs a write to `~/.codex/`, that write is denied, and nothing is stored. The user only finds out in their next session, as `requires OAuth reauthentication` and `MCP startup incomplete (failed: outsystems)`. Wait for the user to confirm they've finished the browser sign-in.
6. **Verify with `codex mcp list`, and read the `Auth` column, not just `Status`.** `Status: enabled` only means the server is registered; a token was stored only if `Auth` reads something other than `Not logged in`. Reporting setup complete on `enabled` alone is the mistake this step exists to prevent. Once `Auth` looks right, **retry the user's original request**.

**On a managed workspace the registration can be refused rather than saved.** ChatGPT Business, Enterprise, and Edu admins can ship an `mcp_servers` allowlist in `requirements.toml`, and Codex enables a server only when both its name and its identity (here, the URL) match an approved entry; anything else is disabled. So if `codex mcp list` doesn't show `outsystems` after an `add` that reported success, or Codex says it fell back to a compatible value, that's workspace policy and not a bad edit. Tell the user to ask their workspace admin to allow the `outsystems` server for `https://<TENANT>/mcp`, and don't re-run the registration or hand-edit `config.toml` to work around it.

## Authenticating

The remote MCP server is OAuth-protected with **standard OAuth** (an unauthenticated call gets `401` + `WWW-Authenticate`, and the server advertises OAuth discovery + DCR: `/authorize`, `/token`, `/register`, PKCE S256). **Authentication is performed by Codex, not by an OutSystems tool — the server exposes no `authenticate` tool, and there is no agent-callable auth tool in Codex.**

Codex runs the flow itself: it opens the system browser and listens on an ephemeral `127.0.0.1` port for the callback. The **user** runs `codex mcp login outsystems` in their own terminal, or makes the first OutSystems tool call and lets Codex prompt. Don't run the login from inside a session: the sign-in appears to succeed and the token is silently not stored, because writing it needs access to `~/.codex/` that your sandbox denies. In the ChatGPT desktop app the same server appears under **Settings > MCP servers** with an **Authenticate** button; in the IDE extension it's the gear menu > **MCP servers**. All three read the one `~/.codex/config.toml`, so a sign-in done on any of them serves the others.

**Needs a local browser.** The callback lands on `127.0.0.1` on the same machine as Codex, so a browser has to be reachable there. On a remote/SSH session with no local browser, run the sign-in where a browser can reach `localhost` and retry.

**If the browser shows "site can't be reached" at the callback URL:** The sign-in already succeeded on the tenant side — the identity provider issued the code and redirected. Only the last local hop failed: Codex's callback listener and the browser's redirect resolved to different addresses, typically an IPv4/IPv6 loopback mismatch in a VM, where the listener is on `[::1]:PORT` and the redirect went to `127.0.0.1:PORT`. **Do not wait for the timeout, and tell the user so immediately** — they don't need to sit through the failure. Ask the user for the full URL from the address bar (it looks like `http://127.0.0.1:PORT/callback/<id>?state=...&code=...`), then **complete the callback yourself via the terminal**: re-send the identical callback to Codex's listener with `curl "http://[::1]:PORT/callback/<id>?state=...&code=..."`, swapping the host to `[::1]` if the address bar showed `127.0.0.1`. That hand-off is not an auth tool call; it simply delivers the code to the loopback listener where the redirect was meant to land. The code is single-use, so do not also ask the user to retry in the browser — one of you, not both. **Expect the attempt to be dead already:** a refused connection tears down the authorization session, so you'll likely find the code spent. If so, ask the user to start a fresh `codex mcp login outsystems` and tell them the corrected address (`[::1]` instead of `127.0.0.1`) to use *before* they click through. Treat authorization codes as credentials either way.

**Reactive.** On `data.category: "AuthError"` mid-session (token expired, refresh denied, etc.): the stored grant lapsed — ask the user to re-run `codex mcp login outsystems` in their own terminal (or hit **Authenticate** in the desktop app), then retry the original call ONCE. If the rejection names `tenant_not_allowed`, do not re-trigger sign-in, see Rules.

**If sign-in fails:** Check the browser first. If it shows "site can't be reached" at the callback URL, see the guidance above — that's a local network issue, not a server fault, and often recoverable by asking the user for the URL. For other failures (server unreachable, DCR fails): surface the message verbatim and file against `OutSystems/outsystems-mcp`. Don't speculate about server internals. One exception: an error saying the OAuth callback port is already in use is a local port conflict, not a server fault, so don't file it. See Troubleshooting at https://github.com/OutSystems/outsystems-mcp#troubleshooting
