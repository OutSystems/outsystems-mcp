...
2. In the Chat tab, ask an OutSystems-related question. If the agent shows no awareness of the skill at all — it doesn't run the Desktop setup subsection in step 3 below and doesn't apply `## Rules` in step 5 — the plugin is not delivering skill content to the Chat tab. That falsifies the `CLAUDE.md` table's Claude Desktop row and `README.md`'s "delivers the same conventions doc" claim: stop, record it as a blocking finding, and do not merge either claim until this is confirmed working.
3. Let the agent drive `skills/outsystems/SKILL.md`'s "For Claude Desktop Users (Manual Config)" subsection end-to-end from a fresh state (tenant prompt, config patch, restart instruction, OAuth sign-in). This is what certifies the skill doc's own Desktop recipe works, not just that Desktop received the plugin. If the agent surfaces the skill but the recipe misleads it, note the specific step and treat it as a doc gap to fix. If the agent cannot run `npx` or read/write `claude_desktop_config.json` at all, do not treat it as a doc gap — it is a harness-capability limit, and it also invalidates `README.md`'s own Desktop paste-prompt recipe (which demands the same capabilities). Record it as a blocking finding, then complete the config edit by hand and continue with steps 4-8.
4. **Second pass, README-documented order.** Uninstall the plugin and remove the `outsystems` config entry again. Wire the MCP server using `README.md`'s Claude Desktop paste prompt, restart Desktop, then reinstall the plugin and restart again. Ask the same kind of OutSystems question. Confirm the skill recognizes the server as already configured and does not re-run its setup recipe — this exercises the order `README.md` actually documents (server wired first, plugin installed after), the inverse of steps 1-3 above. Gaps found here are Desktop-setup-recipe gaps; fix and record per step 8 below.
5. Confirm the skill's `## Rules` are in effect in the Chat tab: trigger a destructive-tenant-operation prompt and observe the confirm-before-destructive behavior.
...
**Teardown.** Uninstall the test plugin and remove the `outsystems` marketplace registration it created. Restore your saved `claude_desktop_config.json` if you saved one at step 1; if you did not, remove the `outsystems` entry this test created. Re-add your production `outsystems` marketplace registration if you removed one.
