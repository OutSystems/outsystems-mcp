...
1. Install the plugin via whichever path above applies. Whichever path you used, confirm the installed content is your branch's — check for wording your branch introduced — before proceeding; options 1 and 2 also register a marketplace named `outsystems`, so remove any pre-existing `outsystems` marketplace registration first to avoid resolving to stale content. If you already have an `outsystems` entry in `claude_desktop_config.json` from a prior test cycle, save a copy of the file and remove that entry — the next step has nothing to exercise otherwise. Restart Desktop. This is the plugin-first state that exercises this PR's new Desktop setup subsection — note that it's the inverse of `README.md`'s documented order (server wired first, plugin installed after), so also repeat step 2 below starting from that order at least once, and confirm the skill recognizes the server as already configured rather than re-running setup unnecessarily.

   The plugin delivers the skill doc only — `.claude-plugin/plugin.json` declares no `mcpServers` block — so a plugin-only install genuinely has no `outsystems` entry; that's by design, not a bug to fix by adding one.
2. In the Chat tab, ask an OutSystems-related question and let the agent drive `skills/outsystems/SKILL.md`'s "For Claude Desktop Users (Manual Config)" subsection end-to-end from that fresh state (tenant prompt, config patch, restart instruction, OAuth sign-in). This is what certifies the skill doc's own Desktop recipe works, not just that Desktop received the plugin. If the agent surfaces the skill but the recipe misleads it, note the specific step and treat it as a doc gap to fix. If the agent cannot run `npx` or read/write `claude_desktop_config.json` at all, stop — that is a harness-capability limit, not a doc gap, and it also invalidates `README.md`'s own Desktop paste-prompt recipe (which demands the same capabilities); record it as a blocking finding rather than a gap, and complete the config edit by hand so you can continue with steps 3-5.
